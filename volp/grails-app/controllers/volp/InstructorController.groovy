...
//saving
    ...
    def employeehome()
    {
        println("I am in employee home..."+params)
    }
    def erpadmission()
    {
        println("I am in erpadmission")
        ArrayList rolelinkslist=new ArrayList<RoleLinks>()
        Login login=Login.findById(session.loginid)
        def roles=login.roles
        for(Role r:roles)
        {
            if(r.roletype.type=="Admission")
            {
                def rolelinks=RoleLinks.findAllByRole(r)
                for(RoleLinks rl:rolelinks)
                {
                    rolelinkslist.add(rl)
                }
            }
        }
        rolelinkslist.sort{it.sort_order}
        session.rolelinkslist=rolelinkslist
        redirect(controller:"Instructor", action: "employeehome")
    }
    def erpregistration()
    {
        println("I am in erpregistration")
        ArrayList rolelinkslist=new ArrayList()
        Login login=Login.findById(session.loginid)
        def roles=login.roles
        for(Role r:roles)
        {
            if(r.roletype.type=="Registration")
            {
                def rolelinks=RoleLinks.findAllByRole(r)
                for(RoleLinks rl:rolelinks)
                {
