...
//        crsList.sort{it.rating}
    ...
    def erplogin()
    {
        println("I am in erp login")
    }
    def processerplogin()
    {
        println("I am in processerplogin..."+params)
        ...
        if(login==null)
        {
            login = Login.findByGrno_empidAndPasswordAndIsloginblocked(username, password, false)
            flagemailorinstitutecode=1
            if (login == null)
            {
        ...
        println("Application type::"+at)
        def rt=RoleType.findAllByApplicationtype(at)   //all modules of ERP
        ...
        for(UserType u:ut)
        {
            if(u.type=="Employee")
            {
                println("I am in Employee....")
                Instructor inst=Instructor.findByEmployee_code(login.grno_empid)
                session.instructor=inst
                println("session.instructor::"+session.instructor)
            }
        }
        session.rt=rt
